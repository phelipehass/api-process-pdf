...
func Test_ProcessData(t *testing.T) {
	...
	assert.Equal(t, "Glória", data[0].District)
	assert.Equal(t, "Rua Marechal Hermes", data[0].Street)
}
...
func Test_ProcessData_List(t *testing.T) {
	...
	assert.Equal(t, "Vila Nova", data[0].District)
	assert.Equal(t, "Rua Augusto Borinelli", data[0].Street)
	...
	assert.Equal(t, "Centro", data[1].District)
	assert.Equal(t, "Rua Jaguaruna", data[1].Street)
	...
	assert.Equal(t, "de Pirabeiraba", data[2].District) //aqui pode dar XABU
	assert.Equal(t, "Rua Alfredo Nielson", data[2].Street)
	...
	assert.Equal(t, "Jardim Paraiso", data[3].District)
	assert.Equal(t, "Rua Sculptor", data[3].Street)
	...
	assert.Equal(t, "Parque Guarani", data[4].District)
	assert.Equal(t, "Rua Maria de Lourdes Goularte", data[4].Street)
	...
	assert.Equal(t, "Iririú", data[5].District)
	assert.Equal(t, "Rua Guaíra", data[5].Street)
}
